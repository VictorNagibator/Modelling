...
def schedule(jobs, sequence):
    ...
    for jid in sequence:
        ...
        for m in range(num_machines):
            if m == 0:
                # Для первой детали на первом станке старт зависит от свободного времени первого станка
                # Если время обработки равно 0 — мы НЕ ставим станок в очередь (не будет блокировать),
                # но старт вычисляем по доступности этого станка (поддерживаем последовательность)
                s = machine_times[0]
            else:
                if times[m] == 0:
            ...
            if times[m] != 0:
                machine_times[m] = f
            # Для первой машины также не блокируем
